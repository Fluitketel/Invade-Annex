//init.sqf:   [] execvm "crew.sqf";
   if (isServer || isDedicated || !hasInterFace) exitWith {Diag_log "I was kicked from the naughtythermal.sqf I am not a true client";};
				...
				if(gunner _vehicle == _x) then
					{
						...
						_name = format ["<t size='0.85' color='#FFFFFF' shadow='1'>%1 %2</t> <img size='0.7' color='#CCFF66' shadow='1' image='a3\ui_f\data\IGUI\Cfg\Actions\getingunner_ca.paa'/><br/> <t size='0.85' color='#FFFFFF'>Heading :<t/> <t size='0.85' color='#ff0000'>%3</t><br/>", _name, (name _x), _Azimuth];//--- bl1p edit removed the target
					}; 
            }
            else
            {
