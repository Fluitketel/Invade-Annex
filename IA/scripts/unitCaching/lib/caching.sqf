...
UCD_fnc_cacheObject = {
	...
	// Fluit: Only cache units in following sides (comment out the ones you don't want to be cached
	if (isNil "CachedSides") then { CachedSides = []; };
	waitUntil {!isNil "PARAMS_CacheAOUnits"};
	if (PARAMS_CacheAOUnits == 1 && !(east in CachedSides)) then {
		CachedSides = CachedSides + [east]; 
	};
	waitUntil {!isNil "PARAMS_CacheDynamicUnits"};
	if (PARAMS_CacheDynamicUnits == 1 && !(resistance in CachedSides)) then { 
		CachedSides = CachedSides + [resistance]; 
	};
	
	if (alive _obj && {local _obj} && {!isPlayer _obj} && (side _obj) in CachedSides) then {
