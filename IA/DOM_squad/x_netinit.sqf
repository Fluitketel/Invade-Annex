...
if (!isDedicated) then {
	[QGVAR(grpswmsg), {player globalChat ((_this select 1) + " " + "has left your group")}] call FUNC(NetAddEventSTO);
	[QGVAR(grpswmsgn), {player globalChat ((_this select 1) + " " + "has joined your group")}] call FUNC(NetAddEventSTO);
};